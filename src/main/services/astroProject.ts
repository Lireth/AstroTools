...
// ---- 集合发现缓存：目录结构很少变化，open/refresh 时强制重新发现 ----
const collectionsCache = new Map<string, CollectionDir[]>()
...
/** 校验所选文件夹是否为 Astro 项目 */
export async function validateAstroProject(root: string): Promise<ProjectValidation> {
  ...
  for (const name of ASTRO_CONFIG_NAMES) {
    if (await pathExists(join(root, name))) return { ok: true }
  }
...
async function extractSite(root: string): Promise<string | undefined> {
  for (const name of ASTRO_CONFIG_NAMES) {
...
async function dirHasMarkdown(dir: string): Promise<boolean> {
  try {
    return await walkForMarkdown(dir, 0)
  } catch {
    return false
  }
}

async function walkForMarkdown(dir: string, depth: number): Promise<boolean> {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory() && entry.name !== 'node_modules') {
      if (await walkForMarkdown(join(dir, entry.name), depth + 1)) return true
    }
  }
  return false
}
 ...
 * 结果按项目根目录缓存（project:open / project:refresh 传 force 重新发现）。
 */
export async function discoverCollections(
  root: string,
  options?: { force?: boolean }
): Promise<CollectionDir[]> {
  if (!options?.force) {
    const cached = collectionsCache.get(root)
    ...
    try {
      for (const entry of await readdir(contentRoot, { withFileTypes: true })) {
        if (entry.isDirectory()) {
          await add(join(contentRoot, entry.name), entry.name)
        }
      }
      // src/content 根目录直接放文章且无子集合时，整体视为一个集合
      if (found.size === 0 && (await dirHasMarkdown(contentRoot))) {
        await add(contentRoot, 'content')
      }
    } catch {
      // 目录读取失败则忽略
    }
  ...
  if (found.size === 0) {
    ...
    if (await dirHasMarkdown(pagesRoot)) {
      await add(pagesRoot, 'pages')
    }
  }

  const result: CollectionDir[] = [...found.entries()]
    .map(([dir, name]) => ({ name, dir }))
    .sort((a, b) => a.name.localeCompare(b.name))
  collectionsCache.set(root, result)
  return result
}
...
export async function readProjectInfo(root: string): Promise<ProjectInfo> {
  ...
  const collectionsDirs = await discoverCollections(root, { force: true })
  const posts = await scanPosts(root, collectionsDirs)
